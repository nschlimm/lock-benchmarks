package org.projectbarbel.benchmarks.locking;
...
public class ReentrantReadWriteLock_Verfification_Benchmark extends LockTestHarness{
    ...
    public void a_measureThroughput_fastreader_1(BenchmarkState state) throws InterruptedException {
        ...
        state.readlock.lock();
        try {
            int[] index = getRandomNumberInRange(1, 0, 99);
            state.publish.set(state.randomStrings.get(index[0]));
        } finally {
    ...
    public void a_measureThroughput_fastwriter_1(BenchmarkState state) throws InterruptedException {
        ...
        try {
            int[] index = getRandomNumberInRange(2, 0, 99);
            String value = state.randomStrings.get(index[0]);
            state.randomStrings.set(index[1], value);
        } finally {
